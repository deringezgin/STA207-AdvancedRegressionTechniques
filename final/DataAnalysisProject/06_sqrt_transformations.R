...
# 6. Applying the sqrt Transformation to response and/or predictor variables
#
#
#
...
### 1. Taking the sqrt of the response variable ###
sqrt_model = lm(sqrt(Y) ~ X1 + X2 + X3 + X4)
summary(sqrt_model)

line_assumption_checks(sqrt(Y), sqrt_model, "Square Root of Alcohol Volume Percentage", "Square Root of Alcohol Volume Percentage")

### 2. Taking the sqrt of the predictor variables ###
sqrt_predictor_model = lm(Y ~ sqrt(X1) + sqrt(X2) + sqrt(X3) + sqrt(X4))
summary(sqrt_predictor_model)
line_assumption_checks(Y, sqrt_predictor_model, "Alcohol Volume Percentage\n[Square Root of Predictor Variables]", "Alcohol Volume Percentage")

### 3. Model with both response and predictor variables as sqrt transformed ###
sqrt_complete_model = lm(sqrt(Y) ~ sqrt(X1) + sqrt(X2) + sqrt(X3) + sqrt(X4))
summary(sqrt_complete_model)
line_assumption_checks(sqrt(Y), sqrt_complete_model, "Square Root of Alcohol Volume Percentage\n[Square Root of Predictor Variables]", "Square Root of Alcohol Volume Percentage")
